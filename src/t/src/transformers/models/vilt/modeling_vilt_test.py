...
class ViltSelfOutput(nn.Module):
    ...
    def __init__(self, config: ViltConfig) -> None:
        ...
        # self.dense = Linear(config.hidden_size, config.hidden_size, r=8)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class ViltLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, output_attentions=False):
        ...
        attn_adapt = self.attn_adapter(self.layernorm_before(hidden_states))

        # first residual connection
        hidden_states = attention_output + hidden_states.to(attention_output.device) + attn_adapt * self.attn_adapter_scale
...
class ViltEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        """ l = [ViltLayer_o(config), ViltLayer(config), ViltLayer_o(config), ViltLayer(config), ViltLayer_o(config), 
             ViltLayer(config), ViltLayer_o(config), ViltLayer(config), ViltLayer_o(config), 
             ViltLayer(config), ViltLayer_o(config), ViltLayer(config)] """
        """ l = [ViltLayer_o(config), ViltLayer_o(config), ViltLayer_o(config), ViltLayer_o(config), ViltLayer_o(config), ViltLayer_o(config),
             ViltLayer_o(config), ViltLayer_o(config), ViltLayer_o(config), ViltLayer(config), ViltLayer(config), ViltLayer(config),]
        self.layer = nn.ModuleList(l) """
