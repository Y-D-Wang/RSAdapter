...
class ViltIntermediate(nn.Module):
    def __init__(self, config: ViltConfig) -> None:
        ...
        # self.dense = Linear(config.hidden_size, config.intermediate_size, r=8)
...
class ViltOutput(nn.Module):
    def __init__(self, config: ViltConfig) -> None:
        ...
        # self.dense = Linear(config.intermediate_size, config.hidden_size, r=8)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class Adapter_mid(nn.Module):
    def __init__(self, D_features, D_mid=48, act_layer=nn.GELU, skip_connect=True):
        super().__init__()
        self.skip_connect = skip_connect
        # D_hidden_features = int(D_features * mlp_ratio)
        self.act = act_layer()
        self.D_fc1 = nn.Linear(D_features, D_mid)
        self.D_fc2 = nn.Linear(D_mid, D_features)
...
class ViltLayer(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.s_adapter = Adapter(config.hidden_size, mlp_ratio=0.25)
        # self.mlp_adapter = Adapter(config.hidden_size, mlp_ratio=0.25, skip_connect=False)
        # self.s_adapter = Adapter_mid(config.hidden_size, D_mid=192, skip_connect=False)
        self.mlp_adapter = Adapter_mid(config.hidden_size, D_mid=192, skip_connect=False)
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, output_attentions=False):
        ...
        # s_adapter = self.s_adapter(self.layernorm_before(hidden_states))
        hidden_states = attention_output + hidden_states.to(attention_output.device) # + s_adapter

        # in ViLT, layernorm is also applied after self-attention
        layer_output = self.layernorm_after(hidden_states)
        mlp_adapter = self.mlp_adapter(layer_output)
        layer_output = self.intermediate(layer_output)

        # second residual connection is done here
        layer_output = self.output(layer_output, hidden_states)
        layer_output = layer_output + mlp_adapter
